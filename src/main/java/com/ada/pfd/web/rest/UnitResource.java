...
/**
 * REST controller for managing {@link com.ada.pfd.domain.Unit}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class UnitResource {
     ...
     */
    @GetMapping("/units")
    public List<Unit> getAllUnits(@RequestParam(required = false) String filter) {
        if ("reactor-is-null".equals(filter)) {
            log.debug("REST request to get all Units where reactor is null");
            return StreamSupport.stream(unitRepository.findAll().spliterator(), false).filter(unit -> unit.getReactor() == null).toList();
        }
