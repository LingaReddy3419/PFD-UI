...
export const Image = () => {
  ...
  return (
    <div>
      <h4 id="image-heading" data-cy="ImageHeading">
            ...
            <tbody>
              {imageList.map((image, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td className="font-size">{image.title}</td>
                  <td className="font-size">{image.description}</td>
                  <td className="font-size">{image.fileName}</td>
                  <td className="font-size">{image.url}</td>
                  <td className="font-size">{image.reactor ? <Link to={`/reactor/${image.reactor.id}`}>{image.reactor.id}</Link> : ''}</td>
                  <td className="text-end buttons-padding">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/image/${image.id}`} size="sm" data-cy="entityDetailsButton" className="table-icon green">
                        <FontAwesomeIcon icon="eye" />{' '}
                        {/* <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.view">View</Translate>
                        </span> */}
                      </Button>
                      <Button tag={Link} to={`/image/${image.id}/edit`} size="sm" data-cy="entityEditButton" className="table-icon blue">
                        <FontAwesomeIcon icon="pencil-alt" />{' '}
                        {/* <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.edit">Edit</Translate>
                        </span> */}
                      </Button>
                      <Button tag={Link} to={`/image/${image.id}/delete`} size="sm" data-cy="entityDeleteButton" className="table-icon red">
